...
#training csv#
    def training_set(c, path):
        ...
        train_number = 1750 # 1:1 # train data : test data
        reshape_number = 351 # 1:1 # train data : test data
        framesize = 320
        ...
        ME_M_8 = train_set[train_set['ID']=='ME_M_7'].head(train_number).copy()


        balanced_data = pd.DataFrame()
        balanced_data = balanced_data.append([AS_F_1, AS_F_3, AS_F_4, AS_F_6, AS_M_1, AS_M_2, AS_M_3, ME_F_1, ME_F_4, ME_F_5, ME_M_4, ME_M_7, ME_M_8])
...
#testing csv#
    def testing_set(c, path):
        ...
        test_number = 600 # 1:1 # train data : test data
        reshape_number = 117 # 1:1 # train data : test data
        framesize = 320
        ...
        balanced_data_t = balanced_data_t.append([AS_F_1_t, AS_F_3_t, AS_F_4_t, AS_F_6_t, AS_M_1_t, AS_M_2_t, AS_M_3_t, ME_F_1_t, ME_F_4_t, ME_F_5_t, ME_M_4_t, ME_M_7_t, ME_M_8_t])
...
# concatenate ##############################
        ...
        outputs = Dense(13, activation='softmax')(dense2)
...
#######################################################################################################
# save result ########################################################################################
def saving_result(t,loss1_1, acc1_1, loss2_1, acc2_1):
    file = open("Result/Result_attack7.txt","a")
    file.write('\n')
...
for i in datasetList:
    ...
    print(i +" data set")
    path =attack_number+"/Segmentedspectrograms_" + str(count)
    csv_path = attack_number+"/data_"+ str(count) +"/merged_"

    loss1, acc1, loss2, acc2 = Multi_CNN(path, csv_path)
    printing_result(i,loss1, acc1, loss2, acc2)
    saving_result(i,loss1, acc1, loss2, acc2)
